...
// 0 => start timestamp, 1 => end timestamp, 2 => voting start, 3 => voting end
@storage_var
func timestamps(index : felt) -> (timestamp: felt) {
}
...
@storage_var
func whitelist_round_end_time() -> (res: felt) {
}
...
func constructor{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    name: felt, symbol : felt, mintLimit : felt, _supply_limit : felt, manager : felt, _payment_method : felt, times_len : felt, times : felt*, _merkle_root : felt, prices_len : felt, prices: felt*, mintLimitForClass_len : felt, mintLimitForClass : felt*, _whitelistRoundTime : felt
) {
...
@view
func returnTimestamps{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    index : felt
) -> (res : felt) {
    let ts : felt = timestamps.read(index);
    return (ts,);
}
...
@view
func returnWlEndTime{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (res: felt) {
    let (time) = whitelist_round_end_time.read();
    return (time,);
}
...
func setTimes{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    index : felt, timestamp : felt
) {
    Ownable.assert_only_owner();
    timestamps.write(index, timestamp);
    return();
}
...
func refund{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    ...
    let startTime : felt = timestamps.read(0); 
    let is_later_than_start : felt = is_le(time, startTime);
    ...
    with_attr error_message("StarkGuardians::too early to refund ") {
        assert is_later_than_start = FALSE;
    }
...
func cancelVote{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    ...
    let (this) = get_contract_address();
    let (time) = get_block_timestamp();
    let startTime : felt = timestamps.read(2); 
    let endTime : felt = timestamps.read(3); 
    let is_after_than_start : felt = is_le(time, startTime);
    let is_early_than_end : felt = is_le(time, endTime);
    //only when voting ongoing
    with_attr error_message("StarkGuardians::too early to cancel your vote ") {
        assert is_after_than_start = FALSE;
    }
    with_attr error_message("StarkGuardians::too late cancel your vote ") {
        assert is_early_than_end = TRUE;
    }
    let user_vote_data : UserVotes = returnUserVote(caller_address); 
    with_attr error_message("StarkGuardians::not voted") {
        assert user_vote_data.isVoted = TRUE;
...
func vote_for_price{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let is_after_than_start : felt = is_le(time, startTime);
    let is_early_than_end : felt = is_le(time, endTime);
    with_attr error_message("StarkGuardians::too early to vote ") {
        assert is_after_than_start = FALSE;
    }
    with_attr error_message("StarkGuardians::too late to vote ") {
        assert is_early_than_end = TRUE;
    }
    let user_vote_data : UserVotes = returnUserVote(caller_address); 
...
func mint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(class : felt, mint_amount : felt, proof_len : felt, proof :felt*) {
    ...
    let (time) = get_block_timestamp();
    let startTime : felt = timestamps.read(0); 
    let endTime : felt = timestamps.read(1); 
    let is_after_than_start : felt = is_le(time, startTime);
    let is_early_than_end : felt = is_le(time, endTime);
    with_attr error_message("StarkGuardians::too early to mint ") {
        assert is_after_than_start = FALSE;
    }
    ...
    claimCheck_byVoteCount(caller_address, is_early_than_end, class, total_minted_count, time);
...
@external
func set_wlEndTime{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(req : felt) {
    Ownable.assert_only_owner();
    whitelist_round_end_time.write(req);
    return ();
}
...
func claimCheck_byVoteCount{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    msg_sender : felt, is_early_than_end : felt, class : felt, total_minted_count: Uint256, time : felt
) {
    ...
    if(isVotesLtSupplyLimit == TRUE){
        with_attr error_message("StarkGuardians::too late to mint ") {
            assert is_early_than_end = TRUE;
        }
        if(class == 0){
            isPublicStarted(total_minted_count, time);
            return();
        }else{
            return();
        }
...
func isPublicStarted{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    total_minted_count : Uint256, time : felt
) {
    ...
    let is_supply_less_than_wl_limit : felt = is_le(_mint_limit_for_whitelist, _new_supply_felt);
    let wlEndTime : felt = returnWlEndTime();
    let is_wl_time_due : felt = is_le(wlEndTime, time);
    let _is_public_round_started : felt = is_supply_less_than_wl_limit + is_wl_time_due;
    let is_public_round_started : felt = is_le(1, _is_public_round_started);
    
    with_attr error_message("StarkGuardians::public round not started yet") {
        assert is_public_round_started = TRUE;
    } 
    return();
}
...
func recursiveSetTimes{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    times: felt*, times_len : felt, index : felt
) {
    if(index == times_len){
        return();
    }
    timestamps.write(index, [times]);
    recursiveSetTimes(times + 1, times_len, index + 1);
    return();
}
